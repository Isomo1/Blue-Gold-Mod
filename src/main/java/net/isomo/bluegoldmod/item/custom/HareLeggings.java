...
public class HareLeggings extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, StatusEffect>())
                    .put(ModArmorMaterial.LEATHER_STRIPS, StatusEffects.JUMP_BOOST).build();

    public HareLeggings(ArmorMaterial material, EquipmentSlot slot, Settings settings) {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()){
            tooltip.add(new TranslatableText("item.bluegoldmod.hare_leggings.tooltip.shift"));
        }else{
            tooltip.add(new TranslatableText("item.bluegoldmod.hare_leggings.tooltip"));
        }
    }
